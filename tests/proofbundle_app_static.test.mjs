...
test('standing block preserves blocked release truth', () => {
  ...
  assert.match(html, /typescript:\s*'local full 300-vector run verified 300\/300/);
  assert.doesNotMatch(html, /remainingBlockers:\s*\[[\s\S]*TypeScript full conformance is 300\/300, verified/);
